...
<div class="container mx-auto">
    ...
    <div class="flex justify-end mb-4">
        <a href="{{ route('quotes.create') }}"
            class="font-semibold px-6 py-2 rounded-md bg-[#FFD700] text-black hover:bg-[#e6c200]">
    ...
    <form method="GET" action="{{ route('quotes.index') }}" class="bg-gray-100 p-6 rounded-lg shadow-md mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="text" name="customer" id="customer" value="{{ request('customer') }}"
                    class="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-[#FFD700] focus:border-[#FFD700]"
        ...
        <div class="mt-4 text-right">
            <button type="submit"
                class="bg-[#FFD700] text-white px-4 py-2 rounded-md shadow hover:bg-[#e6c200] focus:outline-none focus:ring-2 focus:ring-[#e6c200]">
                Filter
            </button>
        </div>
    </form>
                ...
                @forelse ($quotes as $quote)
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="px-6 py-4 text-center flex justify-center gap-4">
                            <a href="{{ route('quotes.show', $quote->id) }}" class="hover:text-gray-700" title="Bekijken"
                                style="color: black;">
                                <i class="fas fa-eye"></i>
                            </a>
                            @if (in_array(auth()->user()->role->name, ['CEO', 'Sales', 'Head Sales']))
                                <a href="{{ route('quotes.edit', $quote->id) }}" class="hover:text-gray-700" title="Bewerken"
                                    style="color: black;">
